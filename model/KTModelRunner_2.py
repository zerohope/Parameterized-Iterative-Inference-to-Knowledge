...
class KTModelRunner(object):
  ...
  def train(self):
    ...
    # w
      ...
      if (epoch + 1) % self.train_conf.valid_epoch == 0 :
        model.eval()
        # 验证
        validate(self,val_data_loader,model,loss_func_temp,q_kn,d,epoch)
  ...
  #  保存学生掌握状态
    ...
    train_df.to_csv('hyper_result/' + self.config.DATASET +'/' + self.config.model.hp_type + '/hyper_lambda,epsilon=' + str(lamb) + ',' + str(epsilon) +'.csv',index=False)
...
def validate(self,val_data_loader,model,loss_func_temp,q_kn, d,epoch):
        print('开始验证')
        ...
        while not val_data_loader.is_end():
            stu_id_val, kn_id_val, score_emb_val, user_k_kc_val, ex_id_val = val_data_loader.next_batch()
