...
class TrainDataLoader(object):
    ...
    def getCommonData(self):
        ...
        return Q,d
    # def read_data_demo_line(self):
    #     for stu_id,q_id,score,kn_id,kn_num,user_k_kc in tqdm.tqdm(itertools.zip_longest(*[train] * 3), desc='loading train data:    ',
    #                                     mininterval=2):
    #         stu_id = int(stu_id.strip().strip(','))
    #         q_id = np.array(q_id.strip().strip(',').split(',')).astype(np.int)
    #         score = np.array(q_id.strip().strip(',').split(',')).astype(np.int)
    #         kn_id = np.array(q_id.strip().strip(',').split(',')).astype(np.int)
    #         kn_num = int(kn_num.strip().strip(','))
    ...
    def next_batch(self):
        ...
        for count in range(self.batch_size):
            ...
            # print('当前第{}条数据：'.format(self.ptr))
            log = self.data[self.ptr + count + 1]
...
class ValTestDataLoader(object):
    ...
    def next_batch(self,train_user_id):
        if self.is_end():
        ...
        stu_id = log['user_id']
        while log['user_id'] == train_user_id:
            self.ptr += 1
            log = self.data[self.ptr]
        ...
        return torch.LongTensor([stu_id]), torch.LongTensor(kn_id), torch.Tensor(score_emb), torch.LongTensor(user_k_kc), torch.LongTensor(ex_id)
        # return torch.IntTensor([stu_id]), torch.IntTensor(kn_id), torch.IntTensor(score_emb), torch.IntTensor(user_k_kc), torch.IntTensor(ex_id)
    def is_end(self):
